'use strict';
angular.module('core.nodes', []).service('Nodes', ['$http', '$rootScope',
    function Nodes($http, $rootScope) {
        var self = this;
        var nodes = [];
        var node = null;

        this.loadNodesParent = function (type, parentkey, func) {
            var url = $rootScope.APIBase + "nodes/" + type + "/parentkey/" + parentkey;
            return this.loadNodesUrl(url, type, func);
        }
        ...
        this.loadNodesUrl = function (url, type, func) {
            var self = this;
            $http({
                method: 'GET',
                url: url,
                ...
                if (fallback.status == 401) {
                    ...
                    $rootScope.setup.user.refreshUser(function () {
                        self.loadNodesUrl(url, type, func);
                    });
                } else {
                    alert("Mislukt om nodes op te halen via "+ url);
                }
            });
        }
        ...
        this.loadNodeByKey = function (key, func) {
            var self = this;
            if (self.node && key && self.node.key == key) {
                //In 'cache'
                if (func) func();
            } else {
                var url = $rootScope.APIBase + "node/getByKey/" + key;
                $http({
                    method: 'GET',
                    url: url,
                    headers: {
                    ...
                    if (fallback.status == 401) {
                        //Refresh needed
                        $rootScope.setup.user.refreshUser(function () {
                            self.loadNodeByKey(key, func);
                        });
                    } else {
                        alert("Mislukt om node " + key + " op te halen");
                    }
                });
            }
        }
        ...
        this.setRelationData = function (sourceId, key, targetId, data, func) {
            ...
            $http({
                ...
                if (fallback.status == 401) {
                    ...
                    $("#melding").modal()
                    $rootScope.melding = {
                        "titel": "Wijzigen niet opgeslagen",
                        "tekst": "Je hebt geen toestemming om deze gegevens te wijzigen. Log opnieuw in of gebruik de eenmalige link opnieuw."
                    }
                }
            });
        }
        ...
        this.checkKey = function(id, key, functionUnique, functionNotUnique){
            $http({
                method: 'GET',
                url: $rootScope.APIBase + "key/check/" + id + "/" + key,
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json',
                    'X-Authorization': 'Bearer ' + $rootScope.setup.user.auth.accesstoken
                ...
                if (response.status == 200) {
                    if (response.data == 0) {
                        if (functionUnique) functionUnique();
                    } else {
                        if (functionNotUnique) functionNotUnique();
                    }
                } else {
                    log(response);
                }
                ...
                if (fallback.status == 401) {
                    //Refresh needed
                    $rootScope.setup.user.refreshUser(function () {
                        self.checkKey(id, key, functionUnique, functionNotUnique);
                    });
                } else {
                    alert("Mislukt om key te checken");
                }
            });
        }
