    ...
    controller: ['$http', '$rootScope', '$scope', '$state', '$stateParams', '$window', 'Nodes', '$location', '$anchorScroll', '$timeout',
        function NodesController($http, $rootScope, $scope, $state, $stateParams, $window, Nodes, $location, $anchorScroll, $timeout) {
            ...
            this.setCustomView = function(){
                let customview = null;
                let view = null;
                if($stateParams.customview && $scope.entity && $scope.entity.customviews){
                    for(let i in $scope.entity.customviews){
                        if($scope.entity.customviews[i].key == $stateParams.customview){
                            view = $scope.entity.customviews[i];
                            $scope.stateparams.customview = view.url;
                            $scope.stateparams.customviewurl = view.url; //customview is already loaded with key, so use customviewurl to know when actual url is loaded
                            $scope.stateparams.title = view.title;
                        }
                    }
                }
                if(!view) $scope.stateparams.customview = null;
            }
